...
template<
    typename _MatrixTy,
    class _ElemTraitsTy>
    using mtx_v_mixing_t = add_mixing<
...
template<
    typename _Ty, 
    class _ElemTraitsTy>
    class matrix_view final : public mtx_v_mixing_t<matrix_view<_Ty>, _ElemTraitsTy>
    {
    static_assert(!std::is_pointer_v<_Ty>, "tvd::matrix_view<_Ty> : no specialization of class for pointer");
public :
...
public :
    ...
    template<size_t size>
        explicit matrix_view(matrix<_Ty, size> const& m) 
            : mtx_v_mixing_t<matrix_view<_Ty>, _ElemTraitsTy>()
            , size_(m.size())
            , col_size_(m.csize())
            , array_(m.data()) 
            ...
            if( (col_size_ || size_) == 0) {
                throw EXCEPTION("matrix_view::matrix_view(matrix<_Ty, size> const&) : (col_size_ || size_) == 0)");
                }
            }
    ...
    matrix_view(_Ty *array, size_t size, size_t col_size = 3)
        ...
        {
        if( (col_size || size) == 0) {
            throw EXCEPTION("matrix_view::matrix_view(...) : (col_size || size) == 0)");
            }
        }
    ...
    vector_t operator [] (size_t const& i) const {
        ...
        for(size_t i = i*col_size, j = 0; i < col_size*(i + 1); i++, j++)
            vector[j] = array[i];
        return vector;
        }
    };
