...
double i_to_rads(int const & angle) {
...
template<
	...
	std::pair<matrix<_Ty, col_size>, matrix<_Ty, col_size> > LU(matrix<_Ty, col_size> const & A) {
		...
		{
			for(int i = k - 1; i < col_size; i++)
				for(int j = i; j < col_size; j++)
					L[j][i] = U[j][i]/U[i][i];
			for(int i = k; i < col_size; i++)
				for(int j = k-1; j < col_size; j++)
...
template<typename _Ty,
	is_arithmetic_t<_Ty> = true>
	void move(_Ty x0, _Ty y0, _Ty x1, _Ty y1, math_types::matrix_3xn_t<_Ty> & m_res) {
...
template<typename _Ty>
	void move(_Ty x, _Ty y, math_types::matrix_3xn_t<_Ty> & m_res) {
		move(0, 0, x, y, m_res);
	}
...
template<
	typename _Ty,
	is_arithmetic_t<_Ty> = true>
	void scale(_Ty k_x, _Ty k_y, math_types::matrix_3xn_t<_Ty> & m_res) {
...
template<
	typename _Ty,
	is_arithmetic_t<_Ty> = true>
	void rotate(short g_ang, _Ty x, _Ty y, math_types::matrix_3xn_t<_Ty> & m_res) {
